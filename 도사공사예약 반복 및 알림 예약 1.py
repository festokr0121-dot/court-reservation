...
def sync_pc_time():
    """Synchronize the PC clock using Windows Time Service."""
    try:
        return os.system("w32tm /resync") == 0
    except Exception:
        return False
...
def detect_people_input_completed():
    r, g, b = fast_pixel_strong(494, 779)
    return (
        abs(r - 96) < 6 and
        abs(g - 85) < 6 and
        abs(b - 85) < 6
...
def click_and_confirm_selection(x, y, timeout=0.5, selected_color=SELECTED_COLOR,
    ...
    for _ in range(2):
        ...
        if skip_if_selected:
            r, g, b = fast_pixel_strong(x, y)
            if color_close(r, g, b, selected_color, tol=8):
                return True
...
# 결제하기 클릭 직후 "이미예약완료" 팝업의 확인 버튼 감지 (검정색 텍스트)
ALREADY_RESERVED_AFTER_PAY_POS = (665, 205)

def detect_already_reserved_after_pay():
    x, y = ALREADY_RESERVED_AFTER_PAY_POS
    r, g, b = fast_pixel_strong(x, y)
    return abs(r-0) < 12 and abs(g-0) < 12 and abs(b-0) < 12

def wait_payment_result(timeout=2.0):
    start = time.time()
    while time.time() - start < timeout:
        if detect_already_reserved_after_pay():
...
def run_reservation(res):
    ...
    while state != STATE_DONE and not emergency_stop:
        ...
        if state == STATE_TIME:
            ...
            tx, ty = selected_pos
            if not click_and_confirm_selection(tx, ty):
                if emergency_stop:
                    return "STOP"
        ...
        # ============================
            ...
            if not click_and_confirm_selection(cx, cy):
                if emergency_stop:
                    return "STOP"
        ...
        # ============================
            ...
            for _ in range(2):
                ...
                if not safe_click(529, 545):
                    ...
                    return "NEXT_RESERVATION"
                if wait_for_usage_type_selected(timeout=0.5):
                    break
                if emergency_stop:
                    return "STOP"
            else:
        ...
        # ============================
            ...
            for _ in range(2):
                if emergency_stop:
                    return "STOP"
                if not safe_click(453, 520):
                    return "STOP"
                if not safe_sleep(0.05):
                    return "STOP"
                if not safe_press("backspace"):
                    return "STOP"
                if not safe_press(str(res["people"])):
                    return "STOP"
                if not safe_sleep(0.05):
                    return "STOP"
                if not safe_press("enter"):
                    return "STOP"
                if wait_for_people_input_completed(timeout=0.5):
                    break
                if emergency_stop:
                    return "STOP"
            else:
                print("[FAIL] 인원 입력 색상 2회 시도 후 미감지 - 다음 예약")
                return "NEXT_RESERVATION"
        ...
        # ============================
            ...
            if pay_result == "ALREADY_RESERVED":
                ...
                safe_click(*ALREADY_RESERVED_AFTER_PAY_POS)
                close_popup_with_esc()
                return "NEXT_RESERVATION"
...
def start_on_enter(event=None, from_auto=False, from_f2=False):
    global emergency_stop, auto_repeat_armed, next_auto_run_at
    global refresh_repeat_enabled, sync_done_for_target, refresh_stop_guard_enabled
    emergency_stop = False
    ...
    if not from_auto and not from_f2:
        if focus_reservation_window() and send_refresh_f5():
            ...
            if f2_mode_var.get() == "auto":
                ...
                if stop_dt is not None and stop_dt <= datetime.datetime.now():
                    status.set("새로고침 종료 시각(-10분)이 이미 지났습니다")
                else:
...
ttk.Entry(sync_input_frame, textvariable=sync_offset_var, width=6).grid(row=0, column=0, sticky="w")
...
ttk.Entry(
    refresh_stop_frame,
    textvariable=refresh_stop_offset_var,
    width=6,
    state="readonly",
...
def save_ui_state():
    data = {
        ...
        "refresh_stop_offset": str(DEFAULT_REFRESH_STOP_OFFSET_MIN),
...
def load_ui_state():
    ...
    refresh_stop_offset_var.set(str(DEFAULT_REFRESH_STOP_OFFSET_MIN))
...
repeat_on_check = ttk.Checkbutton(
    repeat_toggle_frame,
    text="ON",
    variable=auto_on_var,
    command=toggle_auto_on,
)
repeat_off_check = ttk.Checkbutton(
    repeat_toggle_frame,
    text="OFF",
    variable=auto_off_var,
    command=toggle_auto_off,
)
...
def apply_f2_mode_rules(update_status=False):
    if f2_mode_var.get() == "auto":
        ...
        auto_repeat_enabled.set(False)
        if update_status:
            status.set("F2 자동 모드: F5 새로고침 반복 사용")
...
def load_repeat_settings():
    global next_auto_run_at, auto_repeat_armed
    if not os.path.exists(settings_path("repeat_settings.txt")):
        return
    with open(settings_path("repeat_settings.txt"), "r") as f:
        state = f.read().strip()
    if state == "ON":
        auto_on_var.set(True)
        auto_off_var.set(False)
...
def run_auto_repeat_once():
    ...
    status.set(f"🔄 자동 반복 실행 — {now} (주기: {repeat_time_var.get()}초)")
    root.update_idletasks()
...
def auto_repeat_tick():
    global next_auto_run_at, auto_repeat_running, added_cooldown_until
    global auto_repeat_armed, refresh_repeat_enabled
    global sync_done_for_target, refresh_stop_guard_enabled
    ...
    try:
        if f2_mode_var.get() == "auto":
            ...
            if not sync_done_for_target and sync_dt is not None and datetime.datetime.now() >= sync_dt:
                status.set(
                    f"동기화 시각 실행 {'완료' if sync_pc_time() else '실패(w32tm)'}"
                )
            ...
            if not auto_repeat_running and now_ts >= next_auto_run_at:
                ...
                try:
                    ...
                    elif send_refresh_f5():
                        status.set(
                            f"새로고침 실행 {datetime.datetime.now().strftime('%H:%M:%S')} "
                            f"(다음 {repeat_time_var.get()}초 후)"
                        )
                    else:
                        refresh_repeat_enabled = False
                        next_auto_run_at = None
...
def stop_all_automation(reason="⚠ ESC 감지 — 모든 자동 실행 정지"):
    global emergency_stop, auto_repeat_armed, refresh_repeat_enabled
    global refresh_stop_guard_enabled
    emergency_stop = True
    auto_repeat_enabled.set(False)
    auto_on_var.set(False)
    auto_off_var.set(True)
...
def on_f5_refresh(event=None):
    global next_auto_run_at, emergency_stop, refresh_repeat_enabled
    global auto_repeat_armed, sync_done_for_target, refresh_stop_guard_enabled
